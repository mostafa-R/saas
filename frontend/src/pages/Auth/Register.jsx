      ...
      <div className="flex w-full md:w-1/2 items-center justify-center p-6">
        <div className="bg-white w-full max-w-md p-8 rounded-2xl shadow-xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <div className="relative">
                <Building size={20} className={`absolute left-3 top-3 ${getIconColor('role')}`} />
                <select
                  name="role"
                  value={form.role}
                  onChange={handleChange}
                ...
                {touched.role && (
                  <div className="absolute right-3 top-3">
                    {errors.role ? 
                      <AlertCircle size={20} className="text-red-500" /> :
                      form.role && <CheckCircle size={20} className="text-green-500" />
                    }
                  </div>
                )}
              </div>
              {errors.role && touched.role && (
                <p className="text-red-600 text-sm mt-1 flex items-center">
                  <AlertCircle size={16} className="mr-1" />{errors.role}
                </p>
              )}
            </div>
            ...
            <button
              type="submit"
              className="w-full bg-gradient-to-r from-green-600 to-green-700 text-white py-3 rounded-lg font-semibold hover:from-green-700 hover:to-green-800 transform hover:scale-[1.02] transition-all shadow-lg"
            >
              Register
            </button>
          </form>
